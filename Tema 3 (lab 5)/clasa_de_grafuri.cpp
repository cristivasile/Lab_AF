...
public:
	...
	//reads adjacency matrix from input stream
...
int main()
{
	...
	initializeFiles("royfloyd");
	inputFile >> nodeNr;

	Graph graph(nodeNr, true, false);
	graph.readAdjacencyMatrix(inputFile);

	for (auto line : graph.getDistancesBetweenNodes())
		printVector(outputFile, line);
