...
class Graph {
	...
	void tarjanDFS_AC(const int, const int, int&, int&, vector<int>&, vector<int>&, vector<bool>&);
public:
	...
	vector<bool> getArticulationPoints(vector<int>&, int&);
...
vector<bool> Graph::getArticulationPoints(vector<int>& connectedComponentHeads, int& nrAC) {
	list<vector<int>> biconnectedComponents;
	vector<int> visitIndex(nrNodes, -1);  //order of traversal in DFS graph forest
	vector<int> lowestAncestorReachable(nrNodes, 0); //index of lowest ancestor reachable by back edges
	int counter = 0; //auxiliary used in mapping traversal order to visitIndex
	vector<bool> articulationPoints(nrNodes, false);

	for (int i = 0; i < nrNodes; i++)
		if (visitIndex[i] == -1) {
			connectedComponentHeads.push_back(i + 1);
			tarjanDFS_AC(i, -1, counter, nrAC, visitIndex, lowestAncestorReachable, articulationPoints); 		//parent of first node in DFS tree is -1
		}

	return articulationPoints;
}

void Graph::tarjanDFS_AC(int currentNode, const int parent, int& counter, int& nrAC, vector<int>& visitIndex, vector<int>& lowestAncestorReachable, vector<bool>& articulationPoints) {
	...
	for (int neighboringNode : adjacencyLists[currentNode]) {
		...
		if (visitIndex[neighboringNode] == -1) {
			...
			tarjanDFS_AC(neighboringNode, currentNode, counter, nrAC, visitIndex, lowestAncestorReachable, articulationPoints);
...
{
	int nrAC = 0;
	initializeFiles("pamant");
	inputFile >> nodeNr >> edgeNr;

	Graph graph(nodeNr, false, false);
	graph.readEdges(inputFile, edgeNr);

	vector<int> teamHeads;
	vector<bool> vulnerableChildren = graph.getArticulationPoints(teamHeads, nrAC);

	outputFile << teamHeads.size() << "\n";
	printVector(outputFile, teamHeads);

	outputFile << nrAC << "\n";
	for (int i = 0; i < nodeNr; i++)
		if (vulnerableChildren[i])
			outputFile << i + 1 << " ";
